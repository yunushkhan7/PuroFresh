...
export class ReportsComponent implements OnInit {
  ...
  public getdata(sd,ed){
    this.partneshipService.getAlldetails().subscribe(data => {
      console.log(data);
      this.data2 = data;
    })
  }
  ...
  submitthetransaction(){
    ...
    var ids = [];
    for(let data4 of this.innertabledata){
      if(data4.isPaymentPaid == true){
        ids.push(data4.partnerTransactionId)
      }
    }
    const datatosend = {
      "partnerDetailsId": this.id,
      "transactionIds": ids
    }
    console.log(datatosend)
    this.partneshipService.senddatatotransaction(datatosend).subscribe(data => {
      console.log(data);
    })
  }
